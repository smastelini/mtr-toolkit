...
for (b in 1:length(bases)) { ### para b bases
	...
	targets[[b]] <- colnames(dataset)[(ncol(dataset)-n.targets[b]+1):ncol(dataset)]
	...
	x1 <- dataset[,1:(ncol(dataset)-n.targets[b])]
	...
	for(out in 1:folds.num){
		...
		index.t <- 1
		for(step in 1:n.targets[b]){
			print(paste("Target",step))

			col.names.targets <- c()
			for(t in targets[[b]]) {
				col.names.targets <- c(col.names.targets, t)
				col.names.targets <- c(col.names.targets, paste0(t, ".pred.", layers))
			}
			...
			for(k in 1:n.folds.tracking) {
				...
				#print log de layers
				write.csv(predictions.training, paste0(output.dir.drs, "/output_logs/internal_logs/", bases[b], "_", tech, "_training_predictions_EV_fold_", formatC(out, width=2, flag="0"), "_step", step, "_TN_fold", formatC(k, width=2, flag="0"), ".csv"))
				write.csv(predictions.validation, paste0(output.dir.drs, "/output_logs/internal_logs/", bases[b], "_", tech, "_validation_predictions_EV_fold_", formatC(out, width=2, flag="0"), "_step", step, "_TN_fold", formatC(k, width=2, flag="0"), ".csv"))
			...
			actual.folder <- getwd()
			setwd(paste0(output.dir.drs, "/output_logs/internal_logs/"))
			names.perf.log <- as.vector(outer(paste0("RMSE.", targets[[b]]), layers, paste, sep="."))
			...
			lapply(1:n.folds.tracking, function(k) {
				log <- read.csv(paste(getwd(),"/", bases[b], "_", tech, paste0("_validation_predictions_EV_fold_", formatC(out, width=2, flag="0"), "_step", step, "_TN_fold", formatC(k, width=2, flag="0")), ".csv", sep=""), header=TRUE, check.names = FALSE)
					# targets
					for (l in layers){
						for(t in targets[[b]]) {
							folds.log[k, paste0("RMSE.", t, ".", l)] <<- RMSE(log[,t], log[, paste(t, "pred", l, sep=".")])
						}
					}
			})
			...
			write.csv(performance.log, paste0("../layers_errors/DRS_error_", bases[b], "_", tech, "_EV_", out, "_step", step, ".csv"), row.names = FALSE)

			setwd(actual.folder)
			##obter minimos rmse's para cada target nas suas médias
			...
			write.csv(mins.maxs, paste0(output.dir.drs, "/output_logs/layers_errors/", bases[b], "_", tech, "_melhores_camadas", formatC(out, width=2, flag="0"), "_step", step, ".csv"), row.names = F)
			...
			write.csv(predictions.modelling, paste0(output.dir.drs, "/output_logs/EV_raw/", bases[b], "_", tech, "_modelling_predictions_fold", formatC(out, width=2, flag="0"), "_step", step, ".csv"))
			write.csv(predictions.testing, paste0(output.dir.drs, "/output_logs/EV_raw/", bases[b], "_", tech, "_testing_predictions_fold", formatC(out, width=2, flag="0"), "_step", step, ".csv"))
