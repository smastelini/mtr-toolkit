dir.create(paste0(output.dir.mtasd, "/prediction_logs/",tech), showWarnings = FALSE, recursive = TRUE)
	  ...
	  if(showProgress){}else{print(paste0("Fold ", k))}
		...
		for(t in targets[[i]]) {
		  if(showProgress){pb$tick()}else{print(t)}
			...
			x.train[, (names.logs.sg) := predictions.l1.train]
			x.test[, (names.logs.sg) := predictions.l1.test]
			...
			x.train[, (names.logs.sg) := NULL]
			x.test[, (names.logs.sg) := NULL]
		}

		write.csv(data.frame(id=sample.names[test.idx], prediction.log, check.names = FALSE), paste0(output.dir.mtasd, "/prediction_logs/",
							tech, "/predictions_MTASD_", bases[i], paste0("_fold", formatC(k, width=2, flag="0")), ".csv"), row.names = FALSE)
	}
}

#Performance metrics
actual.folder <- getwd()
setwd(paste0(output.dir.mtasd, "/prediction_logs"))
i <<- 1
...
lapply(bases, function(b) {
	...
	lapply(1:folds.num, function(k) {
		log <- read.csv(paste0(getwd(),"/", tech, "/predictions_MTASD_", b, paste0("_fold", formatC(k, width=2, flag="0")),".csv"), header=TRUE)
	...
	write.csv(performance.log, paste0("../performance_MTASD_", tech, "_", b, ".csv"), row.names = FALSE)
	i <<- i + 1
})
setwd(actual.folder)
