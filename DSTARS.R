	  ...
	  #print(paste("Fold Training:", j))
		...
		for(k in 1:n.folds.tracking) {
		  if(showProgress){pb$tick()}else{print(paste("Fold tuning", k))}
			...
			while(!all(converged)) {
			  ...
			  if(showProgress){pb$tick()}else{print(paste("Layer", layer))}
				...
				for(t in targets[[i]]) {
					...
					predictions.validation[, paste(layer,t,sep=".")] <- predict_(regressor, x.validation.tuning, tech, targets[[i]])
			...
			write.csv(predictions.training, paste0(output.dir.dstars, "/output_logs/tuning_raw_logs/", bases[i], "_", tech, "_training_predictions_EV_fold_", formatC(j, width=2, flag="0"), "_TN_fold", formatC(k, width=2, flag="0"), ".csv"))
			write.csv(predictions.validation, paste0(output.dir.dstars, "/output_logs/tuning_raw_logs/", bases[i], "_", tech, "_validation_predictions_EV_fold_", formatC(j, width=2, flag="0"), "_TN_fold", formatC(k, width=2, flag="0"), ".csv"))
		}

		rownames(convergence.tracking) <- 0:(nrow(convergence.tracking)-1)
		write.csv(convergence.tracking, paste0(output.dir.dstars, "/output_logs/convergence_layers_logs/", bases[i], "_", tech, "_convergence_accounting_EV_fold_", formatC(j, width=2, flag="0"), ".csv"))

		convergence.tracking <- convergence.tracking/n.folds.tracking
		convergence.tracking <- as.data.frame(sapply(convergence.tracking, function(z, threshold) z >= threshold, threshold = dstars.phi))

		rownames(convergence.tracking) <- 0:(nrow(convergence.tracking)-1)
		write.csv(convergence.tracking, paste0(output.dir.dstars, "/output_logs/convergence_layers_logs/", bases[i], "_", tech, "_convergence_tracking_EV_fold_", formatC(j, width=2, flag="0"), ".csv"))

		convergence.layers[nrow(convergence.layers)+1,] <- c("modelling", as.numeric(sapply(convergence.tracking, function(z) BBmisc::which.last(z) - 1)))
		write.csv(convergence.layers, paste0(output.dir.dstars, "/output_logs/convergence_layers_logs/", bases[i], "_", tech, "_convergence_layers_EV_fold_", formatC(j, width=2, flag="0"), ".csv"), row.names = F)
		...
		while(!all(max.layers.reached)) {
		  if(showProgress){}else{print(paste("Layer", layer))}
			#print(paste("Layer", layer))
			for(t in targets[[i]]) {
				if(convergence.tracking[as.character(layer),t]) {
			...
			addressing <- convergence.tracking[as.character(layer),]
			addressing <- which(addressing == TRUE)
			chosen.layers[addressing] <- layer

			modelling.set.x[, targets[[i]]] <- predictions.modelling[, paste(chosen.layers, targets[[i]],sep=".")]
			testing.set.x[, targets[[i]]] <- predictions.testing[, paste(chosen.layers, targets[[i]],sep=".")]
		...
		write.csv(predictions.modelling, paste0(output.dir.dstars, "/output_logs/modelling_raw_logs/", bases[i], "_", tech, "_modelling_predictions_fold", formatC(j, width=2, flag="0"), ".csv"))
		write.csv(predictions.testing, paste0(output.dir.dstars, "/output_logs/testing_raw_logs/", bases[i], "_", tech, "_testing_predictions_fold", formatC(j, width=2, flag="0"), ".csv"))
		...
		write.csv(final.predictions, paste0(output.dir.dstars, "/output_logs/testing_final_logs/", bases[i], "_", tech, "_testing_final_predictions_fold", formatC(j, width=2, flag="0"), ".csv"))
	}
}
