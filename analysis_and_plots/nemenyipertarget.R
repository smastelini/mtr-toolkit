...
n.nemenyi <- n.folds*sum(n.targets)
	...
	for(tch in techs) {
		...
		for(b in seq(bases)) {
			...
			for(k in seq(n.folds)) {
				...
				errors <- sapply(targets, function(t,l) RRMSE(l[,t], l[,paste0(t,".pred")]), l = log)
				nemenyi[indx:(indx+n.targets[b]-1), paste(mt,tch,sep=".")] <- errors

				indx <- indx + n.targets[b]
			}
		}
	}
}

write.csv(nemenyi, paste0(log.folder, "/nemenyipertarget.csv"), row.names = F)
